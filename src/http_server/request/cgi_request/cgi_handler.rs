use std::{
    ...
    io::Write,
    path::{Path, PathBuf},
    process::{Command, Stdio}, net::TcpStream,
};
...
use crate::http_server::{
    request::{
        cgi_request::{
            ...
            cgi_response::{parse_cgi_response, convert_cgi_response_to_http},
        },

        request::RequestHandler, static_request::static_handler::StaticRequestHandler,
    },
...
impl CgiRequestHandler {
    pub fn new(
        ...
        CgiRequestHandler { cgi_path, cgi_folder, static_handler }
    }
}


fn get_header_or_empty_string(
    request: &Request<String>,
    header_name: HeaderName
) -> String {
    request.headers().get(header_name).map_or(
        String::from(""),
...
fn run_process(
    ...
    let mut stdin = script_process.stdin.take()
...
impl CgiRequestHandler {
    fn generate_environment_variables(
        ...
        let authorization_data = get_header_or_empty_string(
            request,
        ...
        match authorization_pair {
            Some((scheme, parameters)) => {
                metavariables.insert(
                    CGIMetavariable::AuthType,
                    scheme.to_string()
                );
                metavariables.insert(
                    CGIMetavariable::RemoteUser,
        ...
        let content_type = get_header_or_empty_string(
            request,
            header::CONTENT_TYPE
        );
        if content_type != "" {
        ...
        metavariables.insert(
            CGIMetavariable::GatewayInterface,
            String::from("CGI/1.1")
        );

        metavariables.insert(CGIMetavariable::PathInfo, String::from(""));
        metavariables.insert(
            CGIMetavariable::PathTranslated,
            String::from("")
        );
        ...
        let remote_addr = stream.peer_addr().map_or(
            String::from(""),
        ...
        let host_value = request.headers().get(header::HOST).map_or(
            String::from(""),
            |host| host.to_str().unwrap_or("").to_string()
        );
        let (server_name, server_port) = match host_value.split_once(":") {
            Some((server_name, server_port)) => {
                (server_name.to_string(), server_port.to_string())
            },
        ...
        metavariables.insert(
            CGIMetavariable::ServerProtocol,
            String::from("HTTP/1.0")
        );

        metavariables.insert(
            CGIMetavariable::ServerSoftware,
            String::from("Rust Web/0.0.1")
        );
        ...
        match run_process(script_path, request.body(), envs) {
            ...
            Ok(output) => {
                ...
                match cgi_response {
                    ...
                    Ok(cgi_response) => convert_cgi_response_to_http(
                        stream,
                        &self.static_handler,
                        cgi_response
                    )
...
impl RequestHandler<String> for CgiRequestHandler {
    ...
    fn handle_request(&self, stream: &TcpStream, request: &Request<String>)
        ...
        let folder_path = fs::canonicalize(&self.cgi_folder)
            .expect("CGI folder does not exist");
        let file_path = fs::canonicalize(
            Path::new(&self.cgi_folder).join(file_path)
        );
        let abs_file_path = match file_path {
            ...
            Ok(path) => if path.starts_with(folder_path) {
                path
            } else {
                return Some(
                    generate_error_response(StatusCode::NOT_FOUND)
                )
            },
        };
